...
BEGIN
	IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='mix_produtos' AND column_name='codigo_interno') AND
	   NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='mix_produtos' AND column_name='codigo') THEN
		EXECUTE 'ALTER TABLE mix_produtos RENAME COLUMN codigo_interno TO codigo';
	END IF;
...
DO $$
BEGIN
	IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='ofertas' AND column_name='codigo_interno') AND
	   NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='ofertas' AND column_name='codigo') THEN
		EXECUTE 'ALTER TABLE ofertas RENAME COLUMN codigo_interno TO codigo';
	END IF;
...
DO $$
BEGIN
	IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='pedidos_consolidados' AND column_name='codigo_interno') AND
	   NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='pedidos_consolidados' AND column_name='codigo') THEN
		EXECUTE 'ALTER TABLE pedidos_consolidados RENAME COLUMN codigo_interno TO codigo';
	END IF;
