...
Este diretório contém scripts para migrar os nomes de colunas do banco para a nova padronização usada pela aplicação:
 ...
 - codigo_interno (substitui codigo/codigo_interno)
...
2. Ajustar a aplicação para escrever nas novas colunas (`codigo_interno`, `descricao`, `codigo_ean`). Faça deploy e execute em modo compatível (leitura de ambas colunas, se necessário).
