...
def save_promo_pedidos(engine, pedidos_df):
    ...
    try:
        with engine.begin() as conn:
            pedidos_df.to_sql('pedidos_consolidados', con=conn,
...
def show_gestao_promo_page(engine, base_data_path):
    ...
    LISTA_LOJAS_GLOBAL = ['001', '002', '003', '004', '005', '006',
    ...
    if len(lojas_acesso) > 1:
        selected_loja = st.selectbox("Selecione a loja para digitar os pedidos:",
                                     lojas_acesso, index=None, placeholder="Escolha uma loja...")
    else:
        ...
        if not pedidos_para_salvar.empty:
            ...
            with st.spinner("Salvando pedidos..."):
                if save_promo_pedidos(engine, df_final):
                    ...
                    st.error(
                        "Falha ao salvar os pedidos. Verifique os logs de erro.")
        else:
            st.warning(
