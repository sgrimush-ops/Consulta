...
def save_promo_pedidos(engine, pedidos_df):
    ...
    try:
        with engine.begin() as conn:
            pedidos_df.to_sql('pedidos_consolidados', con=conn, if_exists='append', index=False)
        return True
    except Exception as e:
...
def show_gestao_promo_page(engine, base_data_path):
    ...
    st.markdown("Visualize as promoções ativas/futuras e digite os pedidos para a loja desejada.")
    ...
    if len(lojas_acesso) > 1:
        selected_loja = st.selectbox("Selecione a loja para digitar os pedidos:", lojas_acesso, index=None, placeholder="Escolha uma loja...")
    else:
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "descricao": st.column_config.TextColumn("Produto", width="large", disabled=True),
            ...
            "Qtde Caixas": st.column_config.NumberColumn("Qtde Caixas", min_value=0, step=1)
        },
        hide_index=True,
        ...
        if not pedidos_para_salvar.empty:
            ...
            for _, row in pedidos_para_salvar.iterrows():
                ...
                for loja_global in LISTA_LOJAS_GLOBAL:
                    ...
                    pedido_dict[col_name] = row["Qtde Caixas"] if loja_global == selected_loja else 0
            ...
            with st.spinner("Salvando pedidos..."):
                if save_promo_pedidos(engine, df_final):
                    st.success("Todos os pedidos com quantidade maior que zero foram enviados para aprovação!")
                    st.balloons()
    ...
    st.markdown("---")
    st.info("Caso não tenha produtos em oferta para digitar, é porque eles não pertencem ao estoque abastecido pelo CD15 ou não há estoque disponível (> 0).")
    ...
    with st.expander("❔ Precisa de ajuda ou quer fazer uma observação? Abra um chamado."):
        with st.form("chamado_form_promo", clear_on_submit=True):
            mensagem = st.text_area("Digite sua mensagem para o administrador:", key="msg_promo")
            if st.form_submit_button("Enviar Chamado"):
                if mensagem:
                    username = st.session_state.get('username', 'unknown')
                    assunto = f"Chamado via Tela de Gestão de Promoções"
