...
def merge_with_offers(df_pedidos, df_ofertas):
    ...
    if not df_ofertas.empty:
        ...
        df_merged = pd.merge(df_pedidos, df_ofertas, on='codigo', how='left')
        ...
        # Limpa as colunas originais de data se quiser, ou mantém. Vamos manter apenas as formatadas.
        # df_merged = df_merged.drop(columns=['data_inicio', 'data_final'])
        return df_merged
    else:
...
def get_pedidos_aprovados_download(engine) -> pd.DataFrame:
    ...
    try:
        ...
        # AQUI está a correção: aplicamos a mesma lógica para o download
        df_ofertas = get_offers_data(engine)
        df_pedidos = merge_with_offers(df_pedidos, df_ofertas)
