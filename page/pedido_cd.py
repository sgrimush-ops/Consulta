...
def get_product_history(engine, code):
    ...
    query = text(
        "SELECT * FROM historico_solicitacoes WHERE CAST(codigo_interno AS TEXT) = :code"
    )
    with engine.connect() as conn:
        df = pd.read_sql(query, conn, params={"code": str(code)})
    return df
        ...
        if submitted and code_input:
            ...
            with st.spinner("Buscando..."):
                ...
                if not product_df.empty:
                    ...
                    st.warning(
                        "Produto não encontrado na base de dados (mix_produtos)."
                    )
    ...
    if st.session_state.searched_item:
        ...
        with tab1:
            ...
            if not history_df.empty:
                ...
                st.info(
                    "Este produto não possui histórico de pedidos. Você pode ser o primeiro a solicitar!"
                )
                ...
                placeholder_df.loc[0] = [codigo_produto, item.get("produto", "N/A")] + [
                    0
                ] * len(lojas_cols)
            ...
            if st.form_submit_button("Enviar para Aprovação"):
                if total_cx > 0:
                    pedido_data = {
                        ...
                        "descricao": [item.get("produto", "N/A")],
                    ...
                    st.warning(
                        "Nenhuma quantidade foi digitada. O pedido não foi enviado."
                    )
