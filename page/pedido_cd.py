...
def search_product_by_code(engine, code):
    ...
    query = text("""
        ...
        WHERE CAST(codigo AS TEXT) = :code OR CAST(ean AS TEXT) = :code
    """)
...
def get_product_history(engine, code):
    ...
    query = text("SELECT * FROM historico_solicitacoes WHERE CAST(codigo AS TEXT) = :code")
    with engine.connect() as conn:
        df = pd.read_sql(query, conn, params={"code": str(code)})
    return df
...
def get_future_offers(engine, code):
    ...
    query = text("""
        ...
        WHERE CAST(codigo AS TEXT) = :code AND data_final >= :today
...
def show_pedidos_cd_page(engine, base_data_path):
    ...
    if st.session_state.searched_item:
        ...
        codigo_produto = str(item['codigo'])
        ...
        with tab1:
            ...
            if not history_df.empty:
                ...
                placeholder_cols = ['codigo', 'produto'] + lojas_cols
