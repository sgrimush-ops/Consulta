...
def load_database(base_path, _engine):
    ...
    if not df_mix.empty:
        ...
        if 'Codigo' in df_mix.columns:
            # Garante string limpa para o match
    ...
    if not df_hist.empty:
        ...
        # Padronização de Tipos para Match
        ...
        # Agregação para eliminar duplicatas de linhas
...
def calculate_smart_suggestion(v1_cx, v2_cx, v30_cx, est_cx, pend_cx, emb, dias_cobertura=4):
    ...
    # Converte tudo para Unidades para precisão no cálculo
    # Assumindo que as colunas de entrada são CAIXAS
    v1_un = v1_cx * emb
    v2_un = v2_cx * emb
    v30_un = v30_cx * emb
    est_un = est_cx * emb
    ...
    # Peso: 50% última semana, 30% penúltima, 20% média mensal
    ...
    # Retorna Caixas arredondadas para cima
...
def show_pedidos_page(engine, base_data_path):
    ...
    if prod is not None:
        ...
        sub = pd.DataFrame()
        if not df_hist.empty: 
            # Filtro seguro (string vs string) e indexação por loja
        ...
        for l in LISTA_LOJAS:
            ...
            # Busca direta pelo índice da loja ("001", etc)
            if l in sub.index:
                r = sub.loc[l]
                if isinstance(r, pd.DataFrame): r = r.iloc[0] # Proteção extra contra duplicatas
