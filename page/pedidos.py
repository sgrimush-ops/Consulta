...
def load_database(base_path, _engine):
    ...
    if not df_mix.empty:
        ...
        # Mantém código como string, preservando zeros à esquerda ou letras
        if 'Codigo' in df_mix.columns:
            df_mix['Codigo'] = df_mix['Codigo'].astype(str).str.strip()
    ...
    if not df_hist.empty:
        ...
        # Mantém código e loja como string
        if 'Codigo' in df_hist.columns:
            df_hist['Codigo'] = df_hist['Codigo'].astype(str).str.strip()
        if 'Loja' in df_hist.columns:
            df_hist['Loja'] = df_hist['Loja'].astype(str).str.zfill(3)

        # Agrupa somando apenas colunas numéricas existentes
        ...
        if existing_cols:
            df_hist[existing_cols] = df_hist[existing_cols].apply(pd.to_numeric, errors='coerce').fillna(0)
            df_hist = df_hist.groupby(['Codigo', 'Loja'], as_index=False)[existing_cols].sum()
    ...
    if not df_wms.empty:
        ...
        if col_qtd:
            ...
            if 'Codigo' in df_wms.columns:
                df_wms['Codigo'] = df_wms['Codigo'].astype(str).str.strip()
    ...
    try:
        with _engine.connect() as conn:
            ...
            if not df_ofertas.empty:
                df_ofertas['codigo'] = df_ofertas['codigo'].astype(str).str.strip()
...
def calculate_smart_suggestion(v1_cx, v2_cx, v30_cx, est_cx, pend_cx, emb, dias_cobertura=4):
    if emb <= 0: return 0
    v1_un = float(v1_cx) * emb
    v2_un = float(v2_cx) * emb
    v30_un = float(v30_cx) * emb
    est_un = float(est_cx) * emb
    pend_un = float(pend_cx) * emb
...
def show_pedidos_page(engine, base_data_path):
    ...
    if prod is not None:
        ...
        try:
            ...
            emb = 0 if pd.isna(emb_raw) else int(float(str(emb_raw).replace(',', '.')))
        except:
            emb = 0
        if emb <= 0:
        ...
        if not df_wms.empty:
            ...
            if not w.empty: qtd_cd_un = w['Qtd_CD'].iloc[0]
        ...
        if grade:
            ...
            ed = st.data_editor(
                dfg, hide_index=True, use_container_width=True, key=f"g_{codigo}",
                column_config={
