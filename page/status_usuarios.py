...
def get_user_status_df(engine) -> pd.DataFrame:
    ...
    try:
        query = text(
            "SELECT username, empresa, cargo, ultimo_acesso, status_logado, lojas_acesso FROM users"
        )
    ...
    if 'lojas_acesso' in df_users.columns:
        df_users['lojas_acesso'] = df_users['lojas_acesso'].fillna('')
...
def show_status_page(engine, base_data_path):
    ...
    column_config = {
        ...
        "lojas_acesso": st.column_config.TextColumn("Lojas", disabled=True),
    ...
    cols_table = [
        "Selecionar", "username", "empresa", "cargo", "lojas_acesso",
