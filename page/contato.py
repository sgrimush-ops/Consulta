...
def get_all_users(engine):
    """Busca todos os usuários do sistema (exceto admin)."""
    query = text("""
...
def get_all_fornecedores(engine):
    """Busca todos os fornecedores do sistema."""
    query = text("""
        SELECT username, empresa 
        FROM fornecedores_users 
        ORDER BY empresa, username
    """)
    try:
        with engine.connect() as conn:
...
def show_chat_view(engine, ticket_id, role, username):
    ...
    for _, row in messages.iterrows():
        ...
        with st.chat_message(row['remetente_username'], avatar=avatar):
            ...
            st.caption(
                f"Enviado em: {row['data_envio'].strftime('%d/%m/%Y %H:%M')}")
...
def show_contato_page(engine, base_data_path):
    ...
    if 'selected_ticket_id' in st.session_state:
        ...
        if role == "admin":
            ...
            # Nova seção: Admin pode criar chamados para usuários
            with st.expander("➕ Iniciar Nova Conversa com Usuário", expanded=False):
                st.markdown(
                    "Use esta função para entrar em contato com um usuário ou fornecedor.")

                tipo_contato = st.radio(
                    "Tipo de destinatário:",
                    ["Usuário Interno", "Fornecedor"],
                    horizontal=True,
                    key="tipo_contato"
                )

                if tipo_contato == "Usuário Interno":
                    usuarios = get_all_users(engine)
                    if usuarios:
                    ...
                    fornecedores_df = get_all_fornecedores(engine)
                    if not fornecedores_df.empty:
                        fornecedor_opcoes = [
                            f"{row['empresa']} ({row['username']})"
                        ...
                        # Extrair username
                        usuario_selecionado = fornecedor_selecionado.split(
                            "(")[-1].strip(")")
                    else:
                        st.warning("Nenhum fornecedor encontrado no sistema.")
                        usuario_selecionado = None

                if usuario_selecionado:
                    with st.form("admin_new_ticket_form", clear_on_submit=True):
                        assunto_admin = st.text_input("Assunto")
                        mensagem_admin = st.text_area("Mensagem")

                        if st.form_submit_button("Enviar Mensagem", type="primary"):
                            if assunto_admin and mensagem_admin:
                                # Cria o chamado com o admin como remetente
                                success, new_id = create_new_ticket(
                                    engine,
                                    usuario_selecionado,
                                    assunto_admin,
                                    mensagem_admin
                                ...
                                if success:
                                    # Atualiza a primeira mensagem para mostrar que foi o admin
                                    try:
                                        with engine.begin() as conn:
                                            query = text("""
                                                UPDATE contato_mensagens 
                                                SET remetente_username = :admin_username
                                                WHERE chamado_id = :ticket_id
                                                AND remetente_username = :user_username
                                            """)
                                            conn.execute(query, {
                                                "admin_username": username,
                                                "ticket_id": new_id,
                                                "user_username": usuario_selecionado
                                            })
                                    except Exception:
                                        pass
                                    ...
                                    st.success(
                                        f"Chamado criado com sucesso para {usuario_selecionado}!")
                                    st.rerun()
                                else:
            ...
            st.markdown(
                "##### Chamados Ativos (Aguardando Retorno / Respondidos)")
            if df_open.empty:
